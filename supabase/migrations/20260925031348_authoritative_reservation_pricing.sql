-- Recompute reservation totals from owner-configured rates at insert time.
-- This prevents an authenticated customer from supplying a forged
-- amount_total that staff later trusts at check-in.
alter table public.booking add column if not exists rate_unit_snapshot text;
alter table public.walk_in_booking add column if not exists rate_unit_snapshot text;

create or replace function internal.authoritative_reservation_rate_unit(p_listing_id uuid, p_unit_id uuid)
returns text language sql security definer set search_path = '' as $$
    select case when u.rate_day is not null then u.rate_unit
                else coalesce(c.rate_unit, u.rate_unit) end
      from (select p_listing_id as listing_id, p_unit_id as unit_id) ids
      left join public.court c on c.id = ids.listing_id
      left join public.court_unit_inventory u on u.id = ids.unit_id
       and u.court_id = ids.listing_id and u.is_active and u.inventory_verified
$$;
revoke all on function internal.authoritative_reservation_rate_unit(uuid, uuid) from public, anon, authenticated, service_role;
...
begin
    ...
    if v_rate_unit is null or (v_rate_unit = '/hr' and v_day_rate is null) then
        select c.rate, c.rate_unit
          into v_listing_rate, v_listing_rate_unit
          from public.court c
    ...
    if v_rate_unit = '/set' and v_day_rate is not null then
        ...
        select s.night_rate_starts_at into v_cutoff
        ...
        with segments as (
            ...
            select segment_end - segment_start as covered,
                   greatest(interval '0 seconds', least(segment_end, cutoff_at) - greatest(segment_start, cutoff_at)) as night_covered
              from segments
        )
...
create or replace function internal.set_booking_authoritative_amount()
returns trigger language plpgsql security definer set search_path = '' as $$
begin
    new.rate_unit_snapshot := internal.authoritative_reservation_rate_unit(new.court_listing_id, new.court_unit_inventory_id);
...
$$;
revoke all on function internal.set_booking_authoritative_amount() from public, anon, authenticated, service_role;
drop trigger if exists booking_authoritative_amount on public.booking;
create trigger zz_booking_authoritative_amount
before insert on public.booking
for each row execute function internal.set_booking_authoritative_amount();
...
begin
    ...
    new.amount_paid := coalesce(new.amount_total, 0);
    return new;
end;
$$;
revoke all on function internal.set_walkin_authoritative_amount() from public, anon, authenticated, service_role;
drop trigger if exists walkin_authoritative_amount on public.walk_in_booking;
create trigger zz_walkin_authoritative_amount
before insert on public.walk_in_booking
for each row execute function internal.set_walkin_authoritative_amount();

-- Checkout uses the same authoritative function as reservation inserts.
