...
begin
    if exists (
        select 1 from internal.reservation_resource_slots old_slot
        join public.booking b on b.booking_id = old_slot.booking_id and b.status in ('pending', 'confirmed')
        where not exists (
            select 1 from public.court_unit_inventory u
            join public.court_unit_resource_map m on m.court_unit_id = u.id and m.resource_id = old_slot.resource_id
        ...
        union all
        select 1 from internal.reservation_resource_slots old_slot
        join public.walk_in_booking w on w.walkin_id = old_slot.walkin_id and w.status in ('pending', 'confirmed')
        where not exists (
            select 1 from public.court_unit_inventory u
            join public.court_unit_resource_map m on m.court_unit_id = u.id and m.resource_id = old_slot.resource_id
            join public.physical_court_resource r on r.id = m.resource_id and r.is_active
